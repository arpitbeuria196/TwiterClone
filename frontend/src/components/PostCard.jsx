...
const PostCard = ({ post, editHandle, deleteAPI, likeAPI, addCommentAPI }) => {
  ...
  const [comments, setComments] = useState(post.comments || []); // Handle comments array
  ...
  return (
    <div className="flex bg-white shadow-lg rounded-lg mx-4 md:mx-auto my-8 max-w-xl md:max-w-4xl">
      <div className="flex items-start px-6 py-8">
        <img
          className="w-16 h-16 rounded-full object-cover mr-6 shadow"
          src={post.user.profilePic || "/default-avatar.png"}
          alt="avatar"
        />
        <div>
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-semibold text-gray-900 -mt-1 mr-4">
              {post.user.userName}
            </h2>
          ...
          <div className="w-auto h-96 overflow-hidden mt-4">
            <img src={post.media} alt="Post media" className="w-full h-full object-cover rounded-lg" />
          </div>
            ...
            <div className="flex items-center space-x-6 text-gray-700 text-sm">
              ...
              >
                <svg
                  ...
                  viewBox="0 0 24 24"
                  className={`w-5 h-5 ${isLikedByUser ? "text-red-500" : "text-gray-700"}`}
                  stroke="currentColor"
                >
                  <path
                    strokeLinecap="round"
      ...
      {showCommentSection && (
        ...
        >
          <div
            className="bg-white w-full md:w-96 p-8 rounded-lg shadow-lg max-h-[90vh] overflow-y-auto"

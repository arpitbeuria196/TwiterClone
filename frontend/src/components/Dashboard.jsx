...
const editAPI = async (editId) => {
  ...
  try {
    const post = await axios.put(`http://localhost:8000/post/${editId}`, formData, { 
      withCredentials: true,
...
const deleteAPI = async (editId)=>
{
 ...
 const createPostAPI = async ()=>
 {
  ...
  if(media) formData.append("media",media)
  try {
    const post = await axios.post("http://localhost:8000/post", formData, 
      {
      withCredentials: true,
      headers: { "Content-Type": "multipart/form-data" }, 
    });
 ...
 const mediaHandle = (e) => {
  ...
  return (
    <div className="flex h-screen">
      {/* Left Sidebar */}
      <div className=" fixed top-0 left-0 h-full bg-gray-800 text-white">
        <Sidebar />
      </div>

      <div className="bg-white-100 w-full flex mx-72 mt-2">
  <div className="max-w-3xl w-full h-96 bg-white p-8 rounded-lg shadow-md"> 
    <form className="mb-24">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-gray-500 text-sm">Max 280 characters</span>
           ...
           {/* Post Section - Cards After Form */}
   <div className="mt-6 flex-row-reverse">
      {postSelector?.length ? postSelector.map((post, index) => (
        <PostCard
          key={index}
          post={post}
